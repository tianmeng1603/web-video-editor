  ...
  useEffect(() => {
    if (media.type === "text" && onClipUpdate) {
      ...
      if (textElement && parent && !isEditingRef.current?.has(clip.id)) {
        ...
        if (clip.width !== newWidth) {
          updates.width = newWidth;
        }
        if (clip.height !== newHeight) {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [clip.textStyle?.fontSize, clip.text, clip.id, media.type]);
